...
def nearest_node_to_point(graph: nx.Graph, x: float, y: float) -> Hashable:
    """Return the node closest to a clicked coordinate."""
...
def build_ui_state(graph: nx.Graph, result, step_index: int) -> UIState:
    """Build the node classification for a given cascade stage."""
    ...
    for failed_nodes in failed_by_step[1 : step_index + 1]:
        cascade_failure_nodes |= set(failed_nodes)

    removed_nodes = initial_failure_nodes | cascade_failure_nodes
...
def build_network_figure(graph: nx.Graph, node_frame: pd.DataFrame, *, title: str) -> go.Figure:
    ...
    for status in ["normal", "largest_component", "cascade_failure", "initial_failure"]:
...
def _get_selected_node(graph: nx.Graph, selected_mode: str, load_model: str, sample_size: int, seed: int) -> tuple[Hashable | None, float]:
    nodes = _node_label_options(graph)
    if not nodes:
        return None, 0.0
    if selected_mode == "ランダム故障":
        return select_random_nodes(nodes, 1, seed=seed)[0], 0.0
    if selected_mode == "高負荷ノード故障":
        start = time.perf_counter()
        initial_load = compute_node_load(graph, model=load_model, sample_size=sample_size, seed=seed)
        elapsed_ms = (time.perf_counter() - start) * 1000.0
        return select_high_load_nodes(initial_load, 1)[0], elapsed_ms
    return None, 0.0


def _load_cached_graph(nodes_path: Path, edges_path: Path):
    import streamlit as st

    @st.cache_resource(show_spinner=False)
    def _cached(nodes: str, edges: str) -> nx.Graph:
        return project_road_graph(load_road_graph(nodes, edges, mode="directed"))

    return _cached(str(nodes_path), str(edges_path))
...
def _serialize_graph_for_display(graph: nx.Graph) -> tuple[tuple[tuple[Hashable, float, float, str], ...], tuple[EdgeRow, ...]]:
    simple_graph = project_road_graph(graph)
    node_rows: list[tuple[Hashable, float, float, str]] = []
    for node, data in simple_graph.nodes(data=True):
        node_rows.append(
            (
                ...
                node_label_with_location(simple_graph, node),
            )
        )

    edge_rows: list[EdgeRow] = []
    for index, (u, v) in enumerate(simple_graph.edges()):
        if index >= RESULT_EDGE_LIMIT:
            break
        u_data = simple_graph.nodes[u]
        v_data = simple_graph.nodes[v]
        edge_rows.append(
            (
                float(u_data.get("x", 0.0)),
                float(u_data.get("y", 0.0)),
                float(v_data.get("x", 0.0)),
                float(v_data.get("y", 0.0)),
            )
        )
...
def _build_failure_rows(graph: nx.Graph, result, step_index: int) -> tuple[FailureRow, ...]:
    state = build_ui_state(graph, result, step_index)
    failure_nodes = list(state.initial_failure_nodes | state.cascade_failure_nodes)
...
def _edge_rows_to_figure(
    ...
    *,
    ...
    for status in ["initial_failure", "cascade_failure"]:
    ...
    if show_normal_nodes and normal_rows:
        ...
        if not normal_frame.empty:
            fig.add_trace(
                go.Scattermapbox(
                    lon=normal_frame["lon"],
                    lat=normal_frame["lat"],
                    mode="markers",
                    name="正常ノード",
                    marker=dict(color=STATUS_COLORS["normal"], size=3, opacity=0.25),
                    hoverinfo="skip",
                )
            )
...
def build_result_png_bytes(
    ...
    *,
    ...
    return _build_result_png_cached(
        node_rows,
        edge_rows,
        failed_by_step,
        step_index,
...
def _build_result_display_artifacts(
    ...
    *,
    ...
    map_figure = _edge_rows_to_figure(
        edge_rows,
        failure_rows,
        normal_rows,
        ...
        radius_m=radius_m,
        title=title,
        show_normal_nodes=show_normal_nodes,
    )
    timings.append(("結果地図Figure作成", (time.perf_counter() - start) * 1000.0))

    return (
        ResultDisplayArtifacts(
            failure_frame=_failure_rows_to_dataframe(failure_rows),
            normal_preview_frame=_normal_preview_rows_to_dataframe(normal_rows),
...
def _build_failure_rows_cached(
    ...
    for node_id, lon, lat, label in node_rows:
        if node_id not in failed_nodes:
            continue
        status = "initial_failure" if failed_by_step and node_id in set(failed_by_step[0]) else "cascade_failure"
...
def _build_result_png_cached(
    ...
    center_latitudes: list[float] = []
    center_longitudes: list[float] = []
    circle_lon, circle_lat = _circle_coordinates(center_lat, center_lon, radius_m)
    center_longitudes.extend(circle_lon)
    center_latitudes.extend(circle_lat)
    ax.plot(center_longitudes, center_latitudes, color="#1976d2", linewidth=1.2, zorder=2)
    ...
    cascade_failed = set(_failed_nodes_from_steps(failed_by_step, step_index)) - initial_failed
    ...
    _scatter_failed(cascade_failed, "#d32f2f", "連鎖故障")
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        step_mode = st.checkbox("各 cascade_step を表示", value=False)
    ...
    if len(place_candidates) == 1:
        ...
        selected_place = st.selectbox(
            "候補を選択",
            place_candidates,
            index=min(st.session_state.selected_place_index, len(place_candidates) - 1),
    ...
    radius_m = st.radio(
        ...
        index=0,
        horizontal=True,
        format_func=lambda value: RADIUS_LABELS[value],
    )
    ...
    if selected_mode == "候補ノード選択":
        st.session_state.selected_node = st.selectbox(
            "故障候補ノード",
            node_options,
            index=node_options.index(st.session_state.selected_node) if st.session_state.selected_node in node_options else 0,
            format_func=lambda node: build_selected_node_label(node, simulation_graph),
        )
    ...
    current_signature = build_simulation_cache_key(
        center_lat=selected_place.lat,
        center_lon=selected_place.lon,
        ...
        with st.spinner("カスケード故障を計算しています..."):
            try:
                if current_signature not in simulation_cache:
                    base_timings: list[tuple[str, float]] = [
                        ("道路データ読み込み", st.session_state.last_road_data_load_ms),
                        ("部分グラフ抽出", partial_extract_ms if scope_mode == "部分グラフ" else 0.0),
                        ("初期負荷計算", initial_load_ms),
                    ]
                    ...
                    final_failed_nodes = _failed_nodes_from_steps(failed_by_step, max(len(failed_by_step) - 1, 0))
                    step_index = max(len(failed_by_step) - 1, 0)
                    ...
                    png_bytes = _build_result_png_cached(
                        node_rows,
                        edge_rows,
                        failed_by_step,
                        step_index,
                        execution_center_lat,
                        execution_center_lon,
                        execution_radius_m,
                        f"カスケード故障結果 - ステップ{step_index}",
                        RESULT_EDGE_LIMIT,
                    )
    ...
    step_index = bundle["step_index"] if not step_mode else st.slider(
        "cascade_step",
        min_value=0,
        max_value=max(len(failed_by_step) - 1, 0),
        value=bundle["step_index"],
        step=1,
    )

    if step_mode and step_index != bundle["step_index"]:
        ...
        if display_key not in display_cache:
            ...
            png_bytes = _build_result_png_cached(
                ...
                failed_by_step,
                step_index,
                float(bundle["center_lat"]),
                float(bundle["center_lon"]),
                float(bundle["radius_m"]),
                f"カスケード故障結果 - ステップ{step_index}",
                RESULT_EDGE_LIMIT,
            )
            map_ms = (time.perf_counter() - map_start) * 1000.0
    ...
    metrics = build_state_metrics(simulation_graph, failed_by_step, step_index)
    failed_nodes = _failed_nodes_from_steps(failed_by_step, step_index)
    initial_failed_count = len(failed_by_step[0]) if failed_by_step else 0
    cascade_failed_count = max(len(failed_nodes) - initial_failed_count, 0)
    failure_dataframe = build_failure_dataframe(display_bundle["failure_rows"])
    result_png_bytes = build_result_png_bytes(
        bundle["node_rows"],
        bundle["edge_rows"],
        failed_by_step,
        step_index,
        center_lat=float(bundle["center_lat"]),
        center_lon=float(bundle["center_lon"]),
        radius_m=float(bundle["radius_m"]),
        title=f"カスケード故障結果 - ステップ{step_index}",
        edge_limit=RESULT_EDGE_LIMIT,
    )
    assert isinstance(result_png_bytes, bytes)
    assert result_png_bytes.startswith(b"\x89PNG")
    assert len(result_png_bytes) > 100
    debug_png_path = root / "outputs" / "debug_result_map.png"
    debug_png_path.parent.mkdir(parents=True, exist_ok=True)
    debug_png_path.write_bytes(result_png_bytes)
    print(type(result_png_bytes))
    print(len(result_png_bytes))
    print(f"PNGサイズ: {len(result_png_bytes)} bytes")
    print(f"failure_dataframe rows={len(failure_dataframe)}")
    print(f"failure_dataframe columns={failure_dataframe.columns.tolist()}")
    ...
    if RESULT_DISPLAY_STAGE >= 3:
        ...
        if failure_dataframe.empty:
            ...
            st.dataframe(
                failure_dataframe.head(RESULT_TABLE_LIMIT),
